...
teste, fără obiect `Request` (vezi tests/test_verification.py), iar
fallback-ul "ro" trebuie să rămână identic cu comportamentul verificat de
acele teste.
...
propriul ContextVar, propriul dict T), NU un import comun. verification-
service e stateless (fără MONGO_URL/database.py — vezi app/service.py), dar
tot are un ciclu request/response normal, deci ContextVar-ul funcționează
identic ca în celelalte servicii.
"""
...
T: dict[str, dict[Language, str]] = {
    # Identice cu auth-service/exchange-service/budgets-service — aceleași
    # chei/text, folosite ca convenție de stil (fiecare serviciu validează
    # JWT-ul independent, defense in depth, vezi app/security.py), nu ca o
    # dependență tehnică comună.
    ...
    "imageMissing": {"ro": "Imaginea pentru {label} lipsește.", "en": "The image for {label} is missing."},
    "imageTooLarge": {"ro": "Imaginea pentru {label} e prea mare.", "en": "The image for {label} is too large."},
    # Etichetele imaginilor — traduse separat, ca să nu rămână un cuvânt
    # românesc ("buletin") înfipt într-un mesaj altfel complet în engleză.
    "labelIdDocument": {"ro": "buletin", "en": "ID document"},
    "labelSelfie": {"ro": "selfie", "en": "selfie"},
    "identityUpdateFailedAfterVerification": {
        "ro": "Verificarea a reușit, dar nu am putut actualiza contul. Încearcă din nou.",
        "en": "Verification succeeded, but we could not update the account. Please try again.",
    },
}
