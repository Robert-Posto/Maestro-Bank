...
async def _reject_if_sign_count_regressed(stored_credential: dict, credential: dict[str, Any]) -> None:
    ...
    if stored_count != 0 and new_sign_count <= stored_count:
        db = get_database()
        await db.webauthn_credentials.delete_one({"_id": stored_credential["_id"]})
        logger.error(
...
async def _find_stored_credential(credential: dict[str, Any], *, user_id: str | None = None) -> dict | None:
    ...
    db = get_database()
    query: dict[str, Any] = {"credential_id": base64url_to_bytes(raw_id)}
...
async def begin_registration(user: dict) -> tuple[str, dict[str, Any]]:
    ...
    existing_count = await db.webauthn_credentials.count_documents({"user_id": user_id})
    ...
    existing = await db.webauthn_credentials.find({"user_id": user_id}).to_list(length=_MAX_CREDENTIALS_PER_USER)
...
async def begin_login(email: str) -> tuple[str, dict[str, Any]]:
    ...
    if user is not None:
        user_id = str(user["_id"])
        creds = await db.webauthn_credentials.find({"user_id": user_id}).to_list(length=_MAX_CREDENTIALS_PER_USER)
...
async def finish_login(challenge_id: str, credential: dict[str, Any]) -> tuple[str, dict]:
    ...
    indiferent de metoda de autentificare."""
...
async def revoke_credential(user_id: str, credential_id: str) -> None:
    ...
    result = await db.webauthn_credentials.delete_one({"_id": object_id, "user_id": user_id})
