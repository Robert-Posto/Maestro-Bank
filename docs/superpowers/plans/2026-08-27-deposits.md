...
```
(NOT `""` — unlike exchange/accounts/budgets/support, whose routers mount at root, `deposits-service`'s own router carries `APIRouter(prefix="/deposits", ...)` — see Task 9 — so it needs the SAME `internal_prefix` shape as `transactions-service`'s `"/transactions"` entry, not exchange's empty one. Getting this wrong doesn't fail Task 10's own smoke test — `_is_protected` returns 401 for an unauthenticated request before the URL is ever resolved — it only surfaces once an authenticated request actually reaches the forwarding logic, in Task 11's live verification.)
